...
public abstract class BenchmarkClient implements Runnable {
	...
	protected long startThroughputTime, startLatencyTime, endThroughputTime, elapsed;
	protected String _clientName;
	protected int _highestN;
	protected int _highestDeleted;
 	ArrayList<String> latencies;
	...
	public BenchmarkClient(ZooKeeperBenchmark zkBenchmark, String host, String namespace,
      ...
      latencies = new ArrayList<String>();
	...
	//	}
		...
		LOG.info("Client #" + _id + " -- Current test complete. " +
			"time "+ elapsed+	 " Completed " + _count + " operations.");

   		_zkBenchmark.sendLatencies(latencies);
	...
	void recordEvent(CuratorEvent event) {
		Long submitTime = (Long) event.getContext();
		//long submitTimes = Math.round(submitTime);
	...
	void recordElapsedInterval(long startTime) {
		...
		//	_latenciesFile.write((endtime  -  startTime) + "\n");
