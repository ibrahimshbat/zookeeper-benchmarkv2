...
public class SyncBenchmarkClient extends BenchmarkClient {
	...
	protected void submitWrapped(int n, TestType type) throws Exception {
		...
		for (int i = 0; i < n; i++) {
			long submitTime = (System.nanoTime() - _zkBenchmark.getStartTime())/1000000000;
			...
			switch (type) {
				...
				case CREATE:
					...
					recordElapsedInterval(startLatencyTime);
			...
			if (_syncfin){
				...
				elapsed = (endThroughputTime - startThroughputTime)/1000;
